...
export function LandingStyles() {
  return (
    <style>{`
      ...
      .lnav.scrolled { background:rgba(3,3,16,0.96); backdrop-filter:blur(28px); -webkit-backdrop-filter:blur(28px); border-bottom:1px solid rgba(124,58,237,0.18); }
      ...
      .lnav-link { color:rgba(255,255,255,0.55); font-size:13.5px; padding:8px 14px; border-radius:8px; text-decoration:none; transition:color .2s; font-weight:400; }
      .lnav-link:hover { color:var(--color-text-primary); }

      /* ── Hamburger button ── */
      .lnav-hamburger { display:none; background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.1); border-radius:9px; padding:9px 11px; color:rgba(255,255,255,0.7); cursor:pointer; align-items:center; justify-content:center; transition:background .2s; }
      .lnav-hamburger:hover { background:rgba(255,255,255,0.08); }

      /* ── Mobile menu overlay ── */
      .mobile-menu { position:fixed; top:0; left:0; right:0; bottom:0; z-index:600; background:rgba(3,3,16,0.98); backdrop-filter:blur(24px); -webkit-backdrop-filter:blur(24px); display:flex; flex-direction:column; padding:22px 24px 40px; transform:translateX(100%); transition:transform .3s cubic-bezier(.16,1,.3,1); }
      ...
      .mobile-menu-link { color:rgba(255,255,255,0.6); font-size:22px; font-weight:600; padding:14px 4px; text-decoration:none; border-bottom:1px solid rgba(255,255,255,0.05); transition:color .18s; letter-spacing:-0.4px; font-family:var(--font-display); }
      .mobile-menu-link:hover { color:#F59E0B; }
      .mobile-menu-cta { margin-top:28px; background:linear-gradient(135deg,#F59E0B,#FBBF24); color:#030310; font-weight:700; font-size:16px; padding:17px 28px; border-radius:12px; text-decoration:none; display:flex; align-items:center; justify-content:center; gap:8px; }

      /* ── Hero grid lines ── */
      .hero-grid { position:absolute; inset:0; background-image:linear-gradient(rgba(124,58,237,0.08) 1px,transparent 1px),linear-gradient(90deg,rgba(124,58,237,0.08) 1px,transparent 1px); background-size:72px 72px; mask-image:radial-gradient(ellipse 92% 80% at 50% 50%,black 0%,transparent 100%); -webkit-mask-image:radial-gradient(ellipse 92% 80% at 50% 50%,black 0%,transparent 100%); pointer-events:none; }

      /* ── Hero dot-grid ── */
      .hero-dots { position:absolute; inset:0; background-image:radial-gradient(circle, rgba(228, 221, 209, 0.29) 1.5px, transparent 1.5px); background-size:72px 72px; mask-image:radial-gradient(ellipse 82% 68% at 50% 50%, black 0%, transparent 72%); -webkit-mask-image:radial-gradient(ellipse 82% 68% at 50% 50%, black 0%, transparent 72%); pointer-events:none; opacity:0.28; }
      ...
      /* ── Eyebrow pill ── */
      .eyebrow { display:inline-flex; align-items:center; gap:10px; font-family:var(--font-mono); font-size:10.5px; color:#F59E0B; letter-spacing:2.5px; text-transform:uppercase; border:1px solid rgba(245,158,11,0.22); background:rgba(245,158,11,0.07); padding:8px 18px; border-radius:30px; margin-bottom:36px; animation:fadeUp .8s ease both; }
      .eyebrow-dot { width:5px; height:5px; background:#F59E0B; border-radius:50%; animation:blink 2s infinite; box-shadow:0 0 8px rgba(245,158,11,0.7); }
      ...
      /* ── Quality bars ── */
      .q-bar-fill { height:100%; background:linear-gradient(90deg,currentColor,rgba(255,255,255,0.2)); transform-origin:left; transform:scaleX(0); transition:transform 1.4s cubic-bezier(.16,1,.3,1); border-radius:3px; }

      /* ── Demo input ── */
      .demo-input { width:100%; background:rgba(7,7,28,0.98); border:1.5px solid rgba(255,255,255,0.08); border-radius:11px; color:rgba(255,255,255,0.9); font-size:14px; font-family:var(--font-sans); padding:14px 18px; outline:none; transition:border-color .2s,box-shadow .2s; box-sizing:border-box; }
      .demo-input:focus { border-color:rgba(245,158,11,0.45); box-shadow:0 0 0 3px rgba(245,158,11,0.08); }
      .demo-input::placeholder { color:rgba(255,255,255,0.2); }
      .demo-plat-btn { background:rgba(255,255,255,0.04); border:1px solid rgba(255,255,255,0.07); border-radius:8px; padding:8px 14px; color:rgba(255,255,255,0.42); font-size:12px; font-family:var(--font-sans); cursor:pointer; transition:all .18s; display:flex; align-items:center; gap:6px; }
      .demo-plat-btn.active { background:rgba(245,158,11,0.10); border-color:rgba(245,158,11,0.35); color:#F59E0B; }
      ...
      .feat-new { position:relative; background:rgba(7,7,28,0.98); padding:32px 28px; overflow:hidden; }
      ...
      .hero-stat-item { display:flex; align-items:center; gap:7px; font-family:var(--font-mono); font-size:11px; color:rgba(255,255,255,0.45); letter-spacing:0.4px; padding:0 16px; }
      .hero-stat-item:not(:last-child) { border-right:1px solid rgba(255,255,255,0.07); }
      ...
      .hero-shimmer { background:linear-gradient(90deg,#F59E0B,#FBBF24 32%,#F97316 60%,#F59E0B); background-size:200% auto; -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; animation:shimmer 5s linear infinite; font-style:italic; }
      ...
      @media (max-width:768px) {
        ...
        .step-item{border-left:none !important; border:1px solid rgba(124,58,237,0.14) !important; border-radius:14px !important; background:rgba(7,7,28,0.98) !important; padding:22px 18px !important; transition:none !important}
